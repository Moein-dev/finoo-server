...
router.get("/latest-prices", authenticateToken, async (req, res) => {
    try {
        ...
        return sendErrorResponse(res, 500, "Error retrieving latest prices.");
    }
});
...
router.get("/hourly/:symbol", authenticateToken, async (req, res) => {
    try {
        ...
        console.error(`❌ Error fetching hourly data for ${req.params.symbol}:`, error);
        return sendErrorResponse(res, 500, "Error retrieving hourly data for the specified symbol.");
    }
});
...
router.get("/hourly-data", authenticateToken, async (req, res) => {
    try {
        const { 
        ...
        return sendErrorResponse(res, 500, "Error retrieving hourly data.");
    }
});

// 📌 Get last 24 hours of data for a specific category
router.get("/today/:category", authenticateToken, async (req, res) => {
    try {
...
router.get("/chart", authenticateToken, async (req, res) => {
    try {
        ...
        return sendErrorResponse(res, 500, "Error retrieving chart data.");
    }
});
